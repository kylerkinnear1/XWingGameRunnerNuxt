...
import type { AttackDie } from "#shared/dice";
...
function handleRollDice() {
  const count = Math.max(0, Math.floor(Number(diceCount.value)) || 0);
  const rolledDice = rollAttackDice(count);
  ...
  <div class="roll-attack-dice-component">
    ...
    <div class="dice-preview grid grid-cols-4 gap-4 mb-6">
      <div
        ...
        :key="die.id"
        class="die-button attack-die unrolled"
      >
    ...
    <UButton size="xl" color="error" block @click="handleRollDice">
      <span class="text-lg">Roll Attack Dice</span>
    </UButton>
  </div>
</template>
