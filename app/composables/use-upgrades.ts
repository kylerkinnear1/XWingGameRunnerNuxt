...
export const useUpgrades = () => {
   ...
   */
  function getUpgradesForSlot(
    slotType: string,
    pilotCard: CardPilotDto
  ): UpgradeDto[] {
    ...
    return cards.value.upgrades.filter((upgrade) => {
      ...
      if (
        upgrade.restrictions?.factions &&
        upgrade.restrictions.factions.length > 0
      ) {
      ...
      if (
        upgrade.restrictions?.ships &&
        upgrade.restrictions.ships.length > 0
      ) {
